...
class ReferenceBannerAd: UIView {
    ...
    func load(adm: String?) {
        ...
        bannerAd.load(URLRequest(url: URL(unsafeString: size.rawValue)!)) // swiftlint:disable:this force_unwrapping
    ...
    private func createSingleTap() -> UITapGestureRecognizer {
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(clickthrough))
